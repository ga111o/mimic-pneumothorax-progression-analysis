...
def create_temp_table_and_extract(conn, cohort_df: pd.DataFrame) -> pd.DataFrame:
    temp_table_sql, feature_query = load_sql_file("020_extract_features.sql")
...
def main():
    print("=" * 60)
    print("Pneumothorax Cohort Feature Extraction")
    print("=" * 60)
    ...
    try:
        ...
        output_path = Path(__file__).parent.parent.parent / "data" / "processed" / "cohort_features.csv"
